...
class Issue extends DataObject {
	...
	/**
	 * Get issue cover thumbnail image file name
	 * @param $filename string
	 * @return string
	 */
	function getThumbFileName($filename) {
		$extensionPos = strrpos($filename, '.'); // find position of the last dot, so where the extension starts
		$filename = substr($filename, 0, $extensionPos) . '-t' . substr($filename, $extensionPos);
		return $filename;
	}
