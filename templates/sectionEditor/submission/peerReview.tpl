		...
		{foreach from=$round item=comment}
			{if $smarty.get.showComments && $roundNo == $smarty.get.showComments || !$smarty.get.showComments}
					...
					<table width="100%" class="data">
						...
						<tr>
							<td width="20%" class="label">To</td>
							<td width="80%"><strong>{if $comment.viewable == 1}Author and Editor{else}Editor{/if}</strong></td>
						</tr>
						<tr>
	...
	<table class="data" width="100%">
		<tr id="reviewersHeader" valign="middle">
			...
			<td width="64%" class="nowrap">
				...
				<a href="{url op="selectReviewer" path=$submission->getId()}?f=addAllReviewers" class="action">{translate key="editor.article.selectAllReviewers"}</a>&nbsp;&nbsp;&nbsp;&nbsp;
