	...
	class Pest {
	  ...
	  public function __construct($base_url) {
	    if (!function_exists('curl_init')) {
	   	...
	   	$this->curl_opts['CURLOPT_HTTPPROXYTUNNEL'] = Config::getVar('general', 'proxy_tunnel');
	  ...
	  public function post($url, $data = null) { //default value set by B.Kay 16/08/11	    
	    $curl_opts = $this->curl_opts;
	    $curl_opts[CURLOPT_CUSTOMREQUEST] = 'POST';
	   	if (!is_array($data)) $curl_opts[CURLOPT_HTTPHEADER] = array('Content-Type: text/xml'); //If $data is an array (i.e. files have been sent for upload)
	    $curl_opts[CURLOPT_POSTFIELDS] = $data; //UN..commented out to allow variables to be appended to URL by B.Kay 16/08/11
	  ...
	  public function put($url, $data = Array()) { //default value set by B.Kay 16/08/11
	    $data = (is_array($data)) ? http_build_query($data) : $data;
	    
	    $curl_opts = $this->curl_opts;
	    $curl_opts[CURLOPT_CUSTOMREQUEST] = 'PUT';
	    $curl_opts[CURLOPT_HTTPHEADER] = array('Content-Type: text/xml'); //HTTP Header data added by B.Kay 16/08/11 'Content-Length: '.strlen($data),
	    //$curl_opts[CURLOPT_POSTFIELDS] = $data; //commented out to allow variables to be appended to URL by B.Kay 16/08/11
