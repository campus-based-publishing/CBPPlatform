...
class UserHandler extends Handler {
	...
	function index() {
		...
		if ($journal == null) { // Curently at site level
			...
			$regApproved = $CBPPlatformDao->getUserRegistrationPending(null, $userId);
			foreach ($regApproved as $reg) {
				if (in_array(0, $reg)) {
					$templateMgr->assign('pendingReg', $pending = true);
				}
			}
			...
			while ($journal =& $journals->next()) {
				...
				if (!empty($roles)) {
					$userJournals[$journal->getJournalTitle()] =& $journal;
					$this->getRoleDataForJournal($userId, $journalId, $submissionsCount, $isValid);
				}
				//%CBP% get pending users requests for each journal and push into array
				$pendingUsers[$journalId] = count($CBPPlatformDao->getPendingUserRegistrations($journalId));
		
				//%CBP% if the imprint is classed as a workshop, get workshop articles
				$roles =& $roleDao->getRolesByUserId($userId, $journalId);
				foreach ($roles as $role) {
					$roles[] = $roleDao->getRoleName($role->getRoleId());
				}
				...
				//%CBP% if it's a workshop not within a journa's context
				if ($workshop == 1 && in_array('user.role.reviewer', $roles)) {
					$workshopArticles[$journalId] = count($CBPPlatformDao->getWorkshopArticles($journalId, $userId));
					$templateMgr->assign('workshopArticles', $workshopArticles);
					$workshopArr[$journalId] = 1;
				} else {
					$workshopArr[$journalId] = 0;
				}
				
				$booksForReview[$journalId] = count($CBPPlatformDao->getPendingIssues($journalId));
				$templateMgr->assign('booksForReview', $booksForReview);
				unset($journal);
			}
			ksort($userJournals);
			$templateMgr->assign('workshopArr', $workshopArr);
			...
			$regApproved = $CBPPlatformDao->getUserRegistrationPending($journalId, $userId);
			foreach ($regApproved as $reg) {
				if (in_array(0, $reg)) {
					$templateMgr->assign('pendingReg', $pending = true);
				}
			...
			//%CBP% get pending users requests for journal and push into array
			$pendingUsers[$journalId] = count($CBPPlatformDao->getPendingUserRegistrations($journalId));
			...
			//%CBP% if it's a workshop within a journa's context
			if ($workshop == 1 && in_array('user.role.reviewer', $roles)) {
				$workshopArticles[$journalId] = count($CBPPlatformDao->getWorkshopArticles($journalId, $userId));
				$templateMgr->assign('workshopArticles', $workshopArticles);
				$workshopArr[$journalId] = 1;
		...
		if (isset($isValid["JournalManager"][$journalId])) { 
			if ($isValid["JournalManager"][$journalId] == true) {
				$templateMgr->assign('journalManager', $isValid["JournalManager"][$journalId]);
			}	
		}
		if ($roleDao->getRole(0, $userId, ROLE_ID_SITE_ADMIN)) {
			$pendingIssues = $CBPPlatformDao->getPendingIssues();
			$templateMgr->assign('pendingIssues', count($pendingIssues));
