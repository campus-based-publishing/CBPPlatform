...
class ArticleFileDAO extends DAO {
	...
	function insertArticleFile(&$articleFile) {
		...
		$params = array(
			...
			$articleFile->getFedoraNamespace(), //%LP% update fedora information too
			$articleFile->getFedoraPid(),
			$articleFile->getFedoraDsid()
		);
		...
		$this->update(
			sprintf('INSERT INTO article_files
				(' . ($fileId ? 'file_id, ' : '') . 'revision, article_id, source_file_id, source_revision, file_name, file_type, file_size, original_file_name, type, date_uploaded, date_modified, round, viewable, assoc_id, fedora_namespace, fedora_pid, fedora_dsid)
				VALUES
				(' . ($fileId ? '?, ' : '') . '?, ?, ?, ?, ?, ?, ?, ?, ?, %s, %s, ?, ?, ?, ?, ?, ?)',
