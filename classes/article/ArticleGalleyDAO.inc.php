...
class ArticleGalleyDAO extends DAO {
	...
	function &getGalley($galleyId, $articleId = null) {
		...
		$result =& $this->retrieve(
			'SELECT	g.*,
				a.file_name, a.original_file_name, a.type, a.file_type, a.file_size, a.date_uploaded, a.date_modified, a.fedora_namespace, a.fedora_pid, a.fedora_dsid
	...
	function &getGalleysByArticle($articleId) {
		...
		$result =& $this->retrieve(
			'SELECT g.*,
			a.file_name, a.original_file_name, a.type, a.file_type, a.file_size, a.date_uploaded, a.date_modified, a.fedora_pid, a.fedora_namespace, a.fedora_dsid
			FROM article_galleys g
			LEFT JOIN article_files a ON (g.file_id = a.file_id)
	...
	function insertGalley(&$galley) {
		//%LP% dirty fix
		//TODO clean fix
		$galleyData = $galley->_data;
		$fileId = $galleyData['file_id'];
		$this->update(
