...
class PublishedArticleDAO extends DAO {
	 ...
	 * @return object
	 */
	function &getPublishedArticlesByJournalId($journalId = null, $rangeInfo = null, $reverse = false, $collection = false) {
		$primaryLocale = Locale::getPrimaryLocale();
		$locale = Locale::getLocale();
		$params = array(
			'title',
	 ...
	 * @return PublishedArticle objects array
	 */
	function &getPublishedArticlesInSections($issueId, $useCache = false, $setGalleys = true) {
		/*if ($useCache && $setGalleys) { //%CBP% TODO fix so Cache can be used when setGalleys is false (i.e. for collections)
		...
		while (!$result->EOF) {
			...
			$publishedArticle =& $this->_returnPublishedArticleFromRow($row, null, $setGalleys);
	...
	/**
	 * %CBP% Get published collection (used by imprints that are of 'collection' type)
	 * @param $issueId int
	 * @return $result->fields object
	 */
	function &getPublishedCollection($issueId) {
		$retrieve = "SELECT g.*,
			a.file_name, a.original_file_name, a.type, a.file_type, a.file_size, a.date_uploaded, a.date_modified, a.type, a.fedora_namespace, a.fedora_pid, a.fedora_dsid
			...
			WHERE a.type = 'public/collection' AND
			p.issue_id = $issueId
			ORDER BY g.seq";
		
		//echo $retrieve;
		$result =& $this->retrieve($retrieve);
	 ...
	 * @return PublishedArticle object
	 */
	function &_returnPublishedArticleFromRow($row, $callHooks = true, $setGalleys = true) {
		$publishedArticle = new PublishedArticle();
		$publishedArticle->setPubId($row['pub_id']);
		$publishedArticle->setIssueId($row['issue_id']);
		$publishedArticle->setDatePublished($this->datetimeFromDB($row['date_published']));
		...
		//%CBP% If galleys are set to display
		if ($setGalleys == true) $publishedArticle->setGalleys($this->galleyDao->getGalleysByArticle($row['article_id']));
